...
agent = SwiftAgent(
    ...
    fresh_install=True,
    instruction="""
You are a file searching and reading agent. Remember that file lookups on computer are fuzzy/keyword \
based and not semantic, so try using small chunks for lookups.                
""",
)
...
async def main():
    await agent.run(task="question here")
